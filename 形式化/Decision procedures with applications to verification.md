...
这个新的状态对应于图5.1中的点（B）。$s_3$的下界被违反；这是下一个被选择的基本变量。唯一合适的枢轴变量是$y$。我们需要在$s_3$中添加3以满足下界。这转化为：
...
分配$\alpha(x) = 1, \alpha(y) = 1, \alpha(s_1) = 2, \alpha(s_2) = 1, \alpha(s_3) = 1$。这个赋值$\alpha$满足界限，因此$\{x \mapsto 1, y \mapsto 1\}$是一个满足赋值。它对应于图5.1中的点（C）。

根据基本和非基本变量的固定顺序选择枢轴元素，确保不会重复任何基本变量集，因此保证终止（不会发生循环）。有关详细证明，请参见【109】。这种选择枢轴元素的方法称为Bland规则。