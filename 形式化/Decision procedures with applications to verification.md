...
**例子 3.1** 在T是相等理论的情况下，设计一个简单的$DP_T$是容易的。T-文字要么是某些变量集V上的等式要么是不等式谓词。给定T-文字的合取公式ϕ，构建一个无向图$G(N, E_=, E_≠)$，其中节点N对应于变量V，并且有两种边，$E_=和E_≠$，分别对应于公式ϕ中的等式和不等式谓词。这被称为*等式图*。可以很容易地看出，如果存在一条边$(v1, v2) ∈ E_≠$使得v2可以通过E=边的序列从v1到达，则公式ϕ不可满足。图3.1中的等式图对应于公式$x_1 ≠ x_2 ∧ x_2 = x_3 ∧ x_1 = x_3$。这种程序可以通过在图G上进行|E≠|次深度优先搜索（DFS）调用来实现，因此在输入公式的大小上是多项式时间的。更高效的程序存在，并将在第4章讨论。
...
为了推理具有任意命题结构而不仅仅是合取的公式，我们可以简单地执行分案例（见1.3节），并用DPT决定每个案例。如果任何一个案例是可满足的，那么原始公式也是可满足的。例如，为了决定公式(3.1)，需要考虑四个案例：
