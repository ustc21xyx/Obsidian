...
功能一致性并不总是足以证明正确的陈述。这并不令人惊讶，因为用无解释函数替换具体的解释函数显然会丢失信息。以加法函数“+”为例。假设我们有一个包含两个函数实例$x_1 + y_1$和$x_2 + y_2$的公式，并且由于公式的其他部分，满足$x_1 = y_2$和$y_1 = x_2$。进一步，假设我们用一个二元无解释函数$F$替换“+”。由于我们只比较参数对在出现的顺序中的顺序，证明无法依赖这两个函数实例的计算结果相同。换句话说，仅靠功能一致性无法捕捉“+”函数的交换性，这可能是证明所必需的。这表明，使用无解释函数会丧失完备性（见定义1.6）。
...
未解释的函数可用于基于属性的验证，即证明某个属性在给定模型中成立。有时，无论某个函数的语义如何，属性都是正确的，而功能一致性正是证明所需要的。
不过，未解释函数更常见的用途是证明系统之间的等价性。在芯片设计行业，证明硬件电路两个版本之间的等效性是一项标准程序。另一种应用是翻译验证，即证明编译器的输入和输出在语义上等价的过程。在本章的最后，我们将详细介绍这两个问题领域。

### 4.5.1 证明电路的等价性

流水线是一种提高电路（如微处理器）性能的技术。计算被分成多个阶段，称为流水线阶段。通过并行处理，这允许加速计算，就像工厂中的装配线一样。

电路的时钟频率受限于触发器（即存储组件）之间的最长路径长度，对于流水线电路来说，这只是最长阶段的长度。每条路径的延迟受该路径上的门和每个门施加的延迟的影响。
![](https://raw.githubusercontent.com/ustc21xyx/picture-bed/main/20240518150919.png)
图4.6(a)显示了一个流水线电路。输入（表示为 $in$）在第一阶段进行处理。我们使用无解释函数表示阶段内的组合门，分别记为 $C$、$F$、$G$、$H$、$K$ 和 $D$。为了简化，我们假设它们施加相同的延迟。电路对输入 $in$ 应用函数 $F$，并将结果存储在触发器 $L1$ 中。这可以形式化为：

$$
L_1 = F (in)
$$

第二阶段计算 $L2$、$L3$ 和 $L4$ 的值：
...
第三阶段包含一个多路复用器。多路复用器是根据布尔信号的值在两个输入之间进行选择的电路。在这种情况下，选择信号由函数 $C$ 计算。多路复用器的输出存储在触发器 $L5$ 中：

$$
L5 = C(L2) ? L3 : D(L4)
$$

观察到第二阶段包含两个函数 $G$ 和 $K$，其中 $G$ 的输出用作 $K$ 的输入。假设这是电路内的最长路径。现在我们旨在改造电路以使其运行更快。这可以通过将 $K$ 表示的门下移到第三阶段来完成。

还要注意，$L3$ 和 $L4$ 中的值只有一个被使用，因为多路复用器根据 $C$ 选择其中之一。因此，我们可以通过在第二阶段引入第二个多路复用器来移除一个触发器。更改后的电路如图4.6(b)所示。可以形式化为：
...
计算的最终结果存储在原始电路的 $L5$ 中，并存储在修改后的电路的 $L'5$ 中。我们可以通过证明，对于所有输入，上述等式的联结式蕴含
...
这种证明可以通过使用等式和无解释函数的决策过程自动化。

### 4.5.2 验证编译过程的翻译验证

下一个示例说明了依赖无解释函数的翻译验证过程。与硬件示例不同，我们从解释函数开始，并将其替换为无解释函数。

假设源程序包含如下语句：

$a = (x_1 + y_1) \cdot (x_2 + y_2)$
...
请注意，编译器添加了两个新的辅助变量 $u_1$ 和 $u_2$。为了验证这个翻译，我们构建了验证条件：

$$
u_1 = x_1 + y_1 \wedge u_2 = x_2 + y_2 \wedge z = u_1 \cdot u_2 \implies z = (x_1 + y_1) \cdot (x_2 + y_2)
$$

我们现在将公式中出现的具体函数，即加法和乘法，抽象为无解释函数符号 $F$ 和 $G$。上述蕴含的抽象版本是：

$$
(u_1 = F(x_1, y_1) \wedge u_2 = F(x_2, y_2) \wedge z = G(u_1, u_2)) \implies z = G(F(x_1, y_1), F(x_2, y_2))
$$

显然，如果抽象版本有效，那么原始的具体版本也是有效的。

这种方法的成功取决于两边的差异程度。假设我们正在尝试为一个不执行复杂算术优化的编译器执行翻译验证。在这种情况下，上述方案可能会成功。另一方面，如果我们比较的是两个任意的源代码，即使它们是等价的，可能也不足以使用相同的方案。例如，一边使用函数 $2 \cdot x$ 而另一边使用 $x + x$。由于加法和乘法由两个不同的无解释函数表示，根据函数一致性的要求，它们不会以任何方式相关联，因此无法证明这两个表达式在语义上是相等的。
...
本章介绍线性约束结合的决策过程。回想一下，根据第3章中描述的算法，这些是解决更一般情况下所需的全部内容，即存在任意布尔结构的情况。

**定义 5.1（线性算术）**: 线性算术公式的语法由以下规则定义：
![](https://raw.githubusercontent.com/ustc21xyx/picture-bed/main/20240518155319.png)


二元减法运算符 $a - b$ 可以看作是 $a + (-1) \cdot b$ 的语法糖。如果系数取负，则操作符 ≥ 和 > 可以用 ≤ 和 < 替换。我们考虑有理数和整数作为域。对于前者，该问题是多项式问题，而对于后者，该问题是NP完全的。

例如，以下是一个线性算术公式：

$$
3x_1 + 2x_2 ≤ 5x_3 ∧ 2x_1 - 2x_2 = 0
$$

请注意，等式逻辑（如第4章讨论的）是线性算术的一个片段。以下示例演示了编译器如何使用算术的决策过程来优化代码。
...
该片段旨在将 $a[j]$ 的值复制到位置 $a[j+1]$ 到 $a[j+10]$。编译器可能会为循环体生成汇编代码，如下所示。假设变量 $i$ 存储在寄存器 $R1$ 中，变量 $j$ 存储在寄存器 $R2$ 中：

```
R4 ← mem[a + R2]   // 将 R4 设置为 a[j]
R5 ← R2 + R1       // 将 R5 设置为 j + i
mem[a + R5] ← R4   // 将 a[j+i] 设置为 a[j]
R1 ← R1 + 1        // i++
```

与仅在CPU内部寄存器上操作的代码相比，通常需要内存访问的代码非常慢。因此，非常希望避免加载和存储指令。上述代码的一个潜在优化是将加载指令（即上述的第一条语句）移出循环体。经过此转换，加载指令只在循环开始时执行一次，而不是执行10次。然而，这种转换的正确性依赖于 $a[j]$ 的值在循环体内不会发生变化。我们可以通过将 $a[j + i]$ 的索引与 $a[j]$ 的索引进行比较，并结合 $i$ 在1到10之间的约束，来检查这一条件：

$$
i ≥ 1 ∧ i ≤ 10 ∧ j + i = j
$$

该公式没有可满足的赋值，因此内存访问不可能重叠。编译器可以安全地只对 $a[j]$ 执行一次读取操作。

### 5.1.1 线性算术的求解器

单纯形法是最古老的数值优化算法之一。它用于在给定实变量线性约束的情况下，找到目标函数的最优值。目标函数和约束一起被称为线性规划（LP）。然而，由于我们感兴趣的是决策问题而不是优化问题，因此本章涵盖了一种称为一般单纯形法的变体，该方法输入的是实数上的线性约束的结合，而不需要目标函数，决定这一集合是否可满足。

整数线性规划（ILP）是相同问题在整数约束下的版本。第5.3节介绍了解决这些问题的分支定界算法。

这两种算法可以高效地解决大量约束的结合。我们还将描述两种其他方法，这些方法虽然效率较低，但在解决小问题时仍具有竞争力。

我们描述它们是因为它们在实际中仍然被使用，并且它们的基本形式相对容易实现。第一种方法称为Fourier-Motzkin变量消除，用于决定实数上的线性约束结合的可满足性。第二种方法称为Omega测试，用于决定整数上的线性约束结合的可满足性。

## 5.2 单纯形算法

单纯形算法由丹齐格于1947年提出，用于解决弱线性不等式结合的可满足性。约束集通常伴随一个线性目标函数，以公式的变量表示。如果约束集是可满足的，单纯形算法将提供一个使目标函数值最大的满足赋值。尽管存在多项式时间算法（例如卡钦于1979年提出的椭圆算法），单纯形算法在实际中仍然被广泛使用，并且非常高效，因为在实际问题中，通常不需要指数级的步骤。

### 5.2.1 标准形式

由于我们关注的是决策问题而非优化问题，因此我们将介绍一种不需要目标函数的单纯形算法变体——一般单纯形算法。一般单纯形算法接受两种类型的约束作为输入：

1. 形如 $a_1x_1 + \cdots + a_nx_n = 0$ 的等式。

2. 对变量的上下界约束：$l_i \leq x_i \leq u_i$，其中 $l_i$ 和 $u_i$ 分别表示 $x_i$ 的下界和上界。这些界限是可选的，因为算法支持无界变量。

这种输入公式的表示称为一般形式。这是一种标准形式，不会限制弱线性约束的建模能力，因为我们可以将任意弱线性约束 $L\bowtie R$（其中 $\bowtie∈ {=, ≤, ≥}$ ）转换为上述形式。设 $m$ 为约束的数量，对于第 $i$ 个约束，1 ≤ $i$ ≤ $m$：

1. 将 $R$ 中的所有加项移到左侧，得到 $L' \bowtie b$，其中 $b$ 是常数。

2. 引入一个新变量 $s_i$，添加约束 $L' - s_i = 0$ 和 $s_i \bowtie b$。如果 $\bowtie$ 是等式运算符，将 $s_i = b$ 重写为 $s_i \geq b$ 和 $s_i \leq b$。

原始和转换后的约束结合显然是等价可满足的。


![image.png](https://raw.githubusercontent.com/ustc21xyx/picture-bed/main/20240518160923.png)


新的变量 $s_1, \cdots, s_m$ 被称为**附加变量**。原始约束中的变量 $x_1, \cdots, x_n$ 被称为**问题变量**。因此，我们有 $n$ 个问题变量和 $m$ 个附加变量。作为上述过程的优化，只有在 $L’$ 不是一个问题变量或之前已经分配了一个附加变量时，才引入一个附加变量。

### 5.2.2 单纯形算法的基础

通常，将线性约束满足问题视为几何问题是方便且常见的。从几何角度看，每个变量对应一个维度，每个约束定义了一个凸子空间：特别地，不等式定义了半空间，等式定义了超平面。满足赋值的（闭合）子空间由半空间和超平面的交集定义，形成一个凸多面体。这是由于凸子空间之间的交集也是凸的这一事实所暗示的。图5.1显示了示例5.3中原始问题的几何表示。
![](https://raw.githubusercontent.com/ustc21xyx/picture-bed/main/20240518161227.png)


通常，使用一个$m \times (n + m)$的矩阵$A$来表示输入问题中的系数。变量$x_1, \cdots, x_n, s_1, \cdots, s_m$表示为一个向量$x$。按照这种表示法，我们的问题等价于存在一个向量$x$使得：

$$
Ax = 0 \ \ and\  \ 
\bigwedge_{i=1}^{m} l_i \leq s_i \leq u_i
$$

其中$l_i \in \{-\infty\} \cup \mathbb{Q}$是$x_i$的下界，$u_i \in \{+\infty\} \cup \mathbb{Q}$是$x_i$的上界。无穷值用于表示未设置的界限。

**示例 5.4**: 我们继续示例5.3。使用变量顺序$x, y, s_1, s_2, s_3$，(5.7)中的等式约束的矩阵表示为：
...
请注意，示例5.4中的矩阵有一大部分是非常规则的：为附加变量$s_1, \cdots, s_m$添加的列对应于一个$m \times m$的对角矩阵，其中对角线系数为$-1$。这是使用一般形式的直接结果。

虽然矩阵$A$在算法进行过程中会发生变化，但这种类型的列的数量从未减少。对应于这些列的一组$m$个变量称为**基本变量**，用$B$表示。它们也被称为**依赖变量**，因为它们的值由**非基本变量**的值决定。非基本变量用$N$表示。

存储和操作矩阵$A$的一种表示形式称为表格（tableau），它只是去除了对角子矩阵的$A$。因此，表格是一个$m \times n$的矩阵，其中列对应于非基本变量，每行与一个基本变量相关联——在矩阵$A$的对角子矩阵中，该行对应的基本变量在对角线上有一个“−1”的条目。因此，最初存储在对角矩阵中的信息现在由标记行的变量表示。

**示例 5.5**: 我们继续之前的示例。示例5.3的表格和约束如下：
![](https://raw.githubusercontent.com/ustc21xyx/picture-bed/main/20240518162100.png)

表格只是$A$的不同表示形式，因为$Ax = 0$可以重新写成

$$
\bigwedge_{x_i \in B} \left( x_i = \sum_{x_j \in N} a_{ij} x_j \right)
$$

当写成矩阵形式时，(5.10)右侧的和正好对应于表格。

### 5.2.3 带有上下界的单纯形算法

一般单纯形算法除了保持表格外，还维护一个赋值 $\alpha: B \cup N \rightarrow \mathbb{Q}$。算法初始化其数据结构如下：

- 基本变量集 $B$ 是附加变量的集合。
- 非基本变量集 $N$ 是问题变量的集合。
- 对于任何 $x_i$，其中 $i \in \{1, \cdots, n+m\}$，$\alpha(x_i) = 0$。

如果所有变量初始赋值为零（即原点）满足所有基本变量的上下界，则可以声明该公式是可满足的（回想一下，最初非基本变量没有明确的界限）。否则，算法开始更改这个赋值的过程。

算法5.2.1总结了一般单纯形算法的步骤。算法保持两个不变式：
![](https://raw.githubusercontent.com/ustc21xyx/picture-bed/main/20240518162627.png)
1. $Ax = 0$
2. 非基本变量的值在其界限内：

$$
\forall x_j \in N, \, l_j \leq \alpha(x_j) \leq u_j
$$

显然，这些不变式在初始状态下成立，因为所有变量$x$都设置为0，且非基本变量没有界限。

算法的主循环检查是否存在违反其界限的基本变量。如果没有这样的变量，那么基本和非基本变量都满足其界限。由于不变式1，这意味着当前赋值$\alpha$满足$(5.8)$，并且算法返回“可满足”。

否则，假设$x_i$是一个违反其界限的基本变量，且无损失地假设$\alpha(x_i) > u_i$，即违反了$x_i$的上界。我们如何改变$x_i$的赋值使其满足界限？我们需要找到一种方法来减少$x_i$的值。回想一下这个值是如何指定的：

$$
x_i = \sum_{x_j \in N} a_{ij} x_j
$$

可以通过减少一个非基本变量$x_j$的值来降低$x_i$的值，条件是$a_{ij} > 0$且其当前赋值高于其下界$l_j$，或者通过增加一个变量$x_j$的值，条件是$a_{ij} < 0$且其当前赋值低于其上界$u_j$。满足其中一个条件的变量 $x_j$ 被称为适合的变量。如果没有适合的变量，则问题不可满足，算法终止。

设 $\theta$ 表示需要增加（或减少） $\alpha(x_j)$ 的量，以满足 $x_i$ 的上界：

$$
\theta = \frac{u_i - \alpha(x_i)}{a_{ij}}
$$

增加（或减少） $x_j$ 的值 $\theta$ 使 $x_i$ 满足其界限。另一方面，$x_j$ 可能不再满足其界限，因此可能违反不变式In-2。因此，我们在表格中交换 $x_i$ 和 $x_j$，即使 $x_i$ 成为非基本变量，$x_j$ 成为基本变量。这需要对表格进行转化，称为枢轴操作。枢轴操作重复进行，直到找到满足赋值，或者确定系统不可满足为止。

#### 枢轴操作

假设我们想要交换 $x_i$ 和 $x_j$。我们需要以下定义：

**定义 5.6（枢轴元素、列和行）**：给定两个变量 $x_i$ 和 $x_j$，系数 $a_{ij}$ 被称为枢轴元素。$x_j$ 的列称为枢轴列。行 $i$ 被称为枢轴行。

交换两个变量 $x_i$ 和 $x_j$ 的前提是它们的枢轴元素不为零，即 $a_{ij} \neq 0$。枢轴操作（或枢轴转换）执行如下：

1. 解第 $i$ 行关于 $x_j$ 的方程。
2. 对所有行 $l \neq i$，使用从第 $i$ 行获得的 $x_j$ 的等式消去 $x_j$。

读者可能会注意到，枢轴操作也是众所周知的高斯变量消去过程中的基本操作。

**示例 5.7**：我们继续之前的示例。初始化 $\alpha(x_i) = 0$。这对应于图5.1中的点（A）。回想一下表格和界限：
![](https://raw.githubusercontent.com/ustc21xyx/picture-bed/main/20240518162100.png)


$s_1$ 的下界为2，被违反。顺序中最低的非基本变量是 $x$。变量 $x$ 有正系数，但没有上界，因此适合枢轴操作。我们需要 $s_1$ 增加2 以满足下界，这意味着 $x$ 也需要增加2（$\theta = 2$）。枢轴操作的第一步是解 $s_1$ 的行关于 $x$ 的方程：
...
$$
\begin{aligned}
&s_2 = 2(s_1 - y) - y \iff s_2 = 2s_1 - 3y \\
&s_3 = -(s_1 - y) + 2y \iff s_3 = -s_1 + 3y
\end{aligned}
$$

写成表格形式，枢轴操作的结果是：

![image.png](https://raw.githubusercontent.com/ustc21xyx/picture-bed/main/20240518164346.png)



这个新状态对应于图5.1中的点（B）。$s_3$ 的下界被违反；这是下一个被选择的基本变量。唯一合适的变量是 $y$。我们需要在 $s_3$ 中增加3 以满足下界。这转化为：

$$
\theta = \frac{1 - (-2)}{3} = 1
$$

在对 $s_3$ 和 $y$ 执行枢轴操作后，最终的表如下：
![](https://raw.githubusercontent.com/ustc21xyx/picture-bed/main/20240518164622.png)

这个赋值 $\alpha$ 满足界限，因此 $\{x \mapsto 1, y \mapsto 1\}$ 是一个满足赋值。它对应于图5.1中的点（C）。

根据基本和非基本变量的固定顺序选择枢轴元素，确保不会重复任何基本变量集，因此保证终止（不会发生循环）。这种选择枢轴元素的方法称为Bland规则。

### 5.2.4 增量问题

决策问题通常以增量方式构建，即通过附加的合取项来增强公式。这可能使原本可满足的公式变得不可满足。在DPLL(T)框架中（见第3章），增量决策过程非常有用。

一般单纯形算法非常适合解决增量问题。首先，注意到可以通过去除相应的上下界来禁用任何约束。之后，表格中的等式是冗余的，但不会使可满足的公式变得不可满足。其次，对表格进行的枢轴操作是等价变换，即它保留了解的集合。因此，我们可以从前一组界限中获得的表格开始该过程。

添加上下界的步骤如下：

- 如果为非基本变量添加了一个界限，则根据表格更新非基本变量的值以恢复不变式**In-2**。
- 调用算法5.2.1来确定新问题是否可满足，从第5步开始。

此外，在添加约束后，通常需要移除它们。在DPLL(T)的上下文中，这一点尤为重要，因为该算法会激活和停用约束。通常，当当前约束集不可满足时，会移除约束。移除约束后，需要将赋值恢复到满足一般单纯形算法的两个不变式的状态。这可以通过简单地恢复赋值$\alpha$到最后已知的满足赋值来完成。无需修改表格。

### 5.3 分支定界法

分支定界法是一种广泛使用的方法，用于解决整数线性规划问题。与单纯形算法一样，分支定界法最初是为了解决优化问题而开发的，但这里的描述侧重于该算法在决策问题中的应用。

这里考虑的整数线性系统的形式与5.2节中描述的相同，附加的要求是满足赋值中任何变量的值必须来自整数集合。注意，只需在右侧常数上加1或减1，就很容易支持严格不等式。

**定义 5.8（放宽问题）**：给定一个整数线性系统 $S$，其放宽形式是没有整数性要求的 $S$（即，变量不要求是整数）。

我们用 $relaxed(S)$ 表示 $S$ 的放宽问题。假设存在一个过程 $LPfeasible$，该过程接收一个线性系统 $S$ 作为输入，如果 $S$ 不可满足则返回“不可满足”，否则返回一个满足赋值。$LPfeasible$ 可以通过例如一般单纯形法（算法5.2.1）的变体来实现，如果 $S$ 是可满足的，则输出一个满足赋值。利用这些概念，算法5.3.1可以决定一个整数线性约束系统的可满足性（这里仅考虑约束的合取）。

算法的基本思想很简单：它使用 $LPfeasible$ 解决放宽问题；如果放宽问题不可满足，它会回溯，因为该分支中也没有整数解。另一方面，如果放宽问题可满足并且 $LPfeasible$ 返回的解碰巧是整数解，则算法终止——找到一个满足的整数解。否则，问题被分割成两个子问题，然后通过递归调用处理这些子问题。

**示例 5.9**：设 $x_1, \cdots, x_4$ 是 $S$ 的变量。假设 $LPfeasible$ 在第2行返回解

$$
(1, 0.7, 2.5, 3)
$$

在第7行，$Search-integral-solution$ 选择了 $x_2$ 和 $x_3$ 中的一个作为被赋予非整数值的变量。假设选择了 $x_2$。在第8行，当前递归层次解决的线性系统 $S$ 被增加了约束

$$
x_2 \leq 0
$$

并发送到更深的递归层次。如果在此分支中没有找到解，$S$ 被增加了

$$
x_2 \geq 1
$$

并再次发送到更深的递归层次。如果这两个调用都返回，则表明 $S$ 没有满足解，因此过程返回（回溯）。请注意，从初始递归层次返回会导致调用函数 $Feasibility-Branch-and-Bound$ 返回“不可满足”。

算法5.3.1并不完整：有些情况下它会永远分支。例如，系统 $1 \leq 3x - 3y \leq 2$ 没有整数解但有无界的实数解，这会导致基本分支定界算法永远循环。为了使算法完整，有必要依赖于此类公式具有的小模型性质（我们将在11.6节详细讨论这种性质）。这意味着，如果存在满足解，则在有限界内也存在满足解，对于该理论，这一界限也是可计算的。因此，一旦计算出每个变量的域上的界限，我们就可以在超过该界限后停止搜索解。在优化问题的上下文中，可以在【208】中找到关于此界限的详细研究。这些界限同样适用于可行性问题。简而言之，正如【208】中所示，给定一个具有 $M \times N$ 系数矩阵 $A$ 的整数线性系统 $S$，如果 $S$ 有解，则 $S$ 的凸包的极点之一也是解，并且任何此类解 $x_0$ 的界限如下：

$$
x_{0j} \leq ((M + N) \cdot N \cdot \theta)^N \quad \text{对 } j = 1, \cdots, N
$$

其中 $\theta$ 是问题中的最大元素。因此，(5.21)给出了每个 $N$ 个变量的界限，通过将其添加为显式约束，可以强制终止。

最后，我们提到，分支定界法可以直接扩展到处理某些变量是整数而其他变量是实数的情况。在优化问题的上下文中，这种问题称为**混合整数规划。**