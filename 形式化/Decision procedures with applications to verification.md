...
这些隐含赋值在$B$中没有前提子句，这与标准的布尔约束传播（BCP）不同。这在Analyze-Conflict过程中引发了问题，因为它依赖前提子句来推导冲突子句。为了解决这个问题，当Analyze-Conflict需要这些隐含文字的前提时，它会查询决策程序以获取解释，即部分赋值在创建赋值时隐含该文字的子句。
