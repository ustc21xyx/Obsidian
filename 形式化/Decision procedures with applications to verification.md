...
1. **公式 $t$ 的要求**：
   - $t$ 在理论 $T$ 中是有效的。
   - $t$ 中的原子限制为出现在 $\varphi$ 中的原子。
   - $t$ 的编码与 $\alpha$ 矛盾，即 $e(t)$ 是一个阻塞子句。

2. **优化**：在部分赋值时调用DPT，而不是等待完整赋值。这可以提前排除矛盾的部分赋值，并进行理论传播。

通过这种方式，DPLL(T) 算法能够高效地决定复杂的一阶逻辑公式的可满足性。