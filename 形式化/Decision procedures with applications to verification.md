...
这种输入公式的表示称为一般形式。这是一种标准形式，不会限制弱线性约束的建模能力，因为我们可以将任意弱线性约束 $L ./ R$（其中 ./ ∈ {=, ≤, ≥}）转换为上述形式。设 $m$ 为约束的数量，对于第 $i$ 个约束，1 ≤ $i$ ≤ $m$：

1. 将 $R$ 中的所有加项移到左侧，得到 $L' ./ b$，其中 $b$ 是常数。

2. 引入一个新变量 $s_i$，添加约束 $L' - s_i = 0$ 和 $s_i ./ b$。如果 ./ 是等式运算符，将 $s_i = b$ 重写为 $s_i \geq b$ 和 $s_i \leq b$。
