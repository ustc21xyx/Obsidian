...
以下是“3.4 理论传播与DPLL(T)框架”部分内容的中文翻译，使用专业的计算机术语并力求准确易懂：

### 3.4 理论传播与DPLL(T)框架

#### 3.4.1 传播理论含义

算法3.3.2可以进一步优化。例如，考虑一个包含整数变量 $x_1$ 的公式 $\varphi$，其中包括文字 $x_1 \ge 10$ 和 $x_1 < 0$。

假设Decide过程分配 $e(x_1 \ge 10) \rightarrow \text{true}$ 和 $e(x_1 < 0) \rightarrow \text{true}$。无论做出其他什么决策，任何对Deduction的调用都将导致这两个事实之间的矛盾。然而，算法3.3.2在找到一个完整的满足赋值之前不会调用Deduction。因此，完成赋值所花费的时间是浪费的。此外，如本章介绍中所述，这个完整赋值的驳斥可能是由于其他原因（即证明赋值的不同子集是矛盾的），因此，包含相同错误赋值的其他赋值不会被排除 

算法3.3.2因此可以通过在编码器的完整赋值可用之前运行Deduction来改进。这次提前调用Deduction可以起到两个作用：

1. 及早排除矛盾的部分赋值。
2. 还未分配的文字的含义可以传回SAT求解器，如第3.2节所示。继续我们的例子，一旦 $e(x_1 \ge 10)$ 被分配为 true，我们可以推断出 $e(x_1 < 0)$ 必须为 false，从而避免冲突 

这引出了算法的下一个版本，称为DPLL(T)，它最初由Tinelli以抽象形式引入 。与算法3.3.1和3.3.2一样，算法的组成部分是CDCL和一个用于理论T的共轭片段的决策过程。名称DPLL(T)（如上所述，它也可以称为CDCL(T)）强调这是一个可以实例化为不同理论及其对应决策过程的框架。

#### 3.4.2 性能，性能...

回想一下，当 $\alpha$ 是部分时，Deduction检查理论方面是否存在矛盾，如果不存在，它会执行理论传播。

从性能角度来看，在这一步运行近似查找矛盾通常更好。实际上，只要 $\alpha$ 是部分的，就没有必要进行完整的可满足性判定过程。这不会改变整个算法的完整性，因为当 $\alpha$ 是完整的时，会进行完全检查。一个很好的例子是当理论是整数线性算术时，竞争求解器的做法（将在第5.3节中介绍）。决定这个理论的共轭片段是NP完全的，因此它们只考虑问题的实际松弛（这意味着它们将变量视为实数而不是整数），这可以在多项式时间内解决。这意味着Deduction偶尔会错过一个矛盾，因此不会返回阻塞子句 。

另一个性能考虑与理论传播有关。需要注意的是，理论传播不是为了正确性，而只是为了效率。因此，需要对计算新含义所投入的努力进行精确调整，以实现最佳整体性能 。
...
这些隐含赋值在BCP中没有前置子句，与BCP产生的标准隐含赋值不同。这在Analyze-Conflict中会造成问题，该算法依赖前置子句来推导冲突子句。解决方案是，当Analyze-Conflict需要这些隐含文字的前置子句时，它会查询决策过程以获取解释，即一个由 $\varphi$ 隐含的子句，在创建赋值时根据部分赋值隐含这个文字 。

#### 3.4.4 生成强子句

如果 $T^\wedge h(\alpha)$ 不可满足，Deduction会返回一个阻塞子句 $t$ 以消除赋值 $\alpha$。$t$ 越强，它排除的不一致赋值就越多。获取更强公式的一种方法是构造一个包含那些参与证明 $T^\wedge h(\alpha)$ 不可满足的文字否定的子句。换句话说，如果S是作为证明不可满足前提的文字集合，则阻塞子句为：

$$t := \left( \bigvee_{l \in S} \neg l \right)$$

#### 3.4.5 即时传播

现在考虑这个算法的一种变体，在每次对编码变量进行新赋值后调用Deduction（这可能是由于决策或BCP隐含），而不是让BCP先完成。此外，假设我们正在实现如上所述的完全理论传播。在这个变体中，对Deduction的调用不会导致冲突，这意味着它从不需要返回阻塞子句。这个观察的正式证明留作练习（问题3.5）。一个非正式的理由是，如果对单个编码器的赋值使得 $T^\wedge h(\alpha)$ 不可满足，那么在前一步中该赋值的否定应该已经被Deduction隐含并传播到求解器的布尔部分。