...
算法3.3.2可以进一步优化。考虑一个包含整数变量 $x_1$ 的公式 $ \varphi $，其中包含文字 $x_1 \geq 10$ 和 $x_1 < 0$。

假设Decide过程将 $e(x_1 \geq 10) \mapsto \text{true}$ 和 $e(x_1 < 0) \mapsto \text{true}$。不可避免地，任何对Deduction的调用都会导致这两个事实之间的矛盾，无论做出任何其他决定。然而，算法3.3.2直到找到完整的满足赋值后才调用Deduction。因此，完成赋值所需的时间被浪费了。此外，正如本章介绍中提到的，这个完整赋值的推翻可能是由于其他原因（即证明赋值的不同子集是矛盾的），因此，包含相同错误赋值 $e(x_1 \geq 10)$ 和 $e(x_1 < 0)$ 的其他赋值没有被排除。

因此，算法3.3.2可以通过在编码器的完整赋值可用之前运行Deduction来改进。这种提前调用Deduction可以起到两个作用：

1. 提前排除矛盾的部分赋值。
2. 将仍未分配的文字的蕴涵传达回SAT求解器，如3.2节所示。继续我们的例子，一旦 $e(x_1 \geq 10)$ 被分配为true，我们可以推断 $e(x_1 < 0)$ 必须为false，从而完全避免冲突。

这引出了算法的下一个版本，称为DPLL(T)，它最早由Tinelli以抽象形式引入。与算法3.3.1和3.3.2一样，该算法的组成部分包括CDCL和针对理论 $T$ 的合取片段的决策过程。名称DPLL(T)（如上所述，也可以称为CDCL(T)）强调这是一个可以用不同理论和相应决策过程实例化的框架。

在算法3.4.1（参见图3.3）中展示的DPLL(T)版本中，在BCP无法做出进一步蕴涵后，在第9行调用Deduction。然后，它找到 $T$ 蕴涵的文字，并以约束 $t$ 的形式将它们传达给求解器的CDCL部分。因此，除了布尔域中的蕴涵外，现在还有由于理论 $T$ 的蕴涵。相应地，这种技术被称为理论传播。

#### 3.4.2 性能优化

当 $ \alpha $ 是部分赋值时，Deduction检查理论端是否存在矛盾，如果没有，则执行理论传播。

为了提高性能，在这一步运行一个近似算法以寻找矛盾通常更好。实际上，只要 $ \alpha $ 是部分赋值，就不需要完整的可满足性决策过程。这不会改变整个算法的完备性，因为在 $ \alpha $ 完整时会进行完整的检查。一个很好的例子是竞争求解器在处理整数线性算术时的做法（将在第5.3节中讨论）。决定这一理论的合取片段是NP完全的，因此它们仅考虑问题的实数松弛（这意味着它们将变量视为实数而非整数），这可以在多项式时间内解决。这意味着Deduction有时会错过一个矛盾，因此不会返回阻塞子句。

另一个性能考虑与理论传播有关。需要注意的是，理论传播不是为了正确性，而只是为了效率。因此，为计算新蕴涵而投入的努力需要很好地调整，以实现最佳整体性能。

#### 3.4.3 返回蕴涵赋值而非子句

理论传播的另一种优化涉及到Deduction发现的信息如何传播到求解器的布尔部分。到目前为止，我们要求Deduction返回的子句 $t$ 是 $T$-有效的。例如，如果 $ \alpha $ 是使得 $ T^h(\alpha) $ 蕴涵一个文字 $lit$，则
$$
t := (lit \lor \neg T^h(\alpha))
$$
编码的子句 $e(t)$ 的形式为
$$
e(t) := (e(lit) \lor \bigvee_{lit' \in Th(\alpha)} \neg e(lit'))
$$
然而，Nieuwenhuis、Oliveras 和 Tinelli 认为这是一个低效的方法 。他们在各种基准上的实验表明，这些子句中平均不到0.5%再次使用，并且它们会增加处理负担。他们建议使用更好的替代方法，其中Deduction返回一个包含 $e(lit)$ 的蕴涵赋值列表，SAT求解器然后执行这些赋值。

这些蕴涵赋值在 $B$ 中没有前提子句，这与标准的BCP蕴涵形成对比。这在Analyze-Conflict中引起了问题（见算法2.2.2），该算法依赖前提子句来推导冲突子句。作为解决方案，当Analyze-Conflict需要此类蕴涵文字的前提时，它会查询决策过程以获取解释，即一个由 $\varphi$ 蕴涵的子句，该子句在创建赋值时根据部分赋值推导出该文字。