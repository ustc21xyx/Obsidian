...
回想一下，每个理论 $T$ 都是相对于一个签名 $\Sigma$ 定义的，这是一组允许的符号。例如，在等式理论的情况下，$\Sigma = \{'='\}$。当我们写 $\Sigma$-文字（或类似的 $\Sigma$-原子和 $\Sigma$-公式）时，这意味着该文字只使用 $\Sigma$ 中的符号。

令 $\text{at}(\varphi)$ 表示给定的NNF公式 $\varphi$ 中的 $\Sigma$-原子集合。假设对原子有某种预定义的顺序，我们将 $\varphi$ 中的第 $i$ 个不同的原子记为 $\text{at}_i(\varphi)$。

对于给定的原子 $a$，我们将其与一个唯一的布尔变量 $e(a)$ 关联，我们称之为该原子的布尔编码器。将这个想法扩展到公式，对于给定的 $\Sigma$-公式 $t$， $e(t)$ 表示通过用其布尔编码器替换 $t$ 中的每个 $\Sigma$-原子而得到的布尔公式。例如，如果 $x = y$ 是一个 $\Sigma$-原子，那么 $e(x = y)$ 是一个布尔变量，表示它的编码器。
...
$$
对于一个 $\Sigma$-公式 $\varphi$，所得的布尔公式 $e(\varphi)$ 称为 $\varphi$ 的命题骨架。

使用这种符号表示法，我们现在可以开始概述本章研究的方法，同时跟随一个简单的例子。在这个例子中，我们将继续使用等式理论。
...
$$
令 $B$ 是一个布尔公式，初始设置为 $e(\varphi)$，即
$$
B := e(\varphi)
$$
下一步，我们将 $B$ 传递给一个SAT求解器。假设该公式是可满足的，并且SAT求解器返回满足赋值
$$
\alpha := \{e(x = y) \mapsto \text{true}, e(y = z) \mapsto \text{true}, e(x = z) \mapsto \text{false}\}
$$
这个公式
$$
T^h(\alpha) := x = y \land y = z \land \neg(x = z)
$$
是不可满足的，这意味着这个公式的否定是一个永真式。因此，$B$ 与 $e(\neg T^h(\alpha))$（这个永真式的布尔编码）结合：
$$
e(\neg T^h(\alpha)) := (\neg e(x = y) \lor \neg e(y = z) \lor e(x = z))
$$
这一子句与当前的赋值相矛盾，因此阻止了它的重复。这样的子句称为阻塞子句。一般来说，我们用 $t$ 表示DPT返回的公式——也称为引理。在这个例子中，$t := \neg T^h(\alpha)$，即引理是完整赋值 $\alpha$ 的否定，因此它是一个子句，但通常 $t$ 可以是多个子句，这取决于DPT的实现。

加入阻塞子句后，SAT求解器再次被调用并提出另一个赋值，例如：
$$
\alpha' := \{e(x = y) \mapsto \text{true}, e(y = z) \mapsto \text{true}, e(x = z) \mapsto \text{true}\}
$$
相应的 $\Sigma$-公式
$$
T^h(\alpha') := x = y \land y = z \land x = z
$$
是可满足的，这证明了原始公式 $\varphi$ 是可满足的。实际上，任何满足 $T^h(\alpha')$ 的赋值也满足 $\varphi$。

信息在SAT求解器和用于 $\Sigma$-文字合取的决策程序DPT之间交换。

这种基本程序有许多改进，有些将在本章后面介绍，有些在第3.5节的练习中留下作为练习。例如，调用决策程序DPT可以在某些或所有部分赋值后进行，而不是等待完整赋值。一个矛盾的部分赋值导致一个更强大的引理 $t$，因为它阻止了所有扩展它的赋值。此外，当部分赋值不矛盾时，它可以用于推导出理论传播，并传递回SAT求解器。继续上述例子，考虑部分赋值
$$
\alpha := \{e(x = y) \mapsto \text{true}, e(y = z) \mapsto \text{true}\}
$$
和传递给DPT的相应公式
$$
T^h(\alpha) := x = y \land y = z
$$
这使得DPT得出 $x = z$ 是隐含的，并相应地通知SAT求解器当前部分赋值 $\alpha$ 隐含 $e(x = z) \mapsto \text{true}$。因此，除了SAT求解器执行的正常布尔约束传播（BCP）外，还有理论传播。这种传播可能导致进一步的BCP，这意味着在SAT求解器做出下一个决策之前，这个过程可能会迭代几次。

在接下来的几节中，我们将描述上述过程的变体。
