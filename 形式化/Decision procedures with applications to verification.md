...
AllDifferent的传播器应该检测该约束在这些域下不能满足。SAT中的传播器等价于布尔约束传播（BCP）。传播器必须是健全的，但对于某些约束，使它们完全健全是计算上过于昂贵的（见定义1.6）。换句话说，它可能找不到所有可能的域缩减，并且可能在部分赋值下不总是检测到冲突。这并不改变整个求解器的完整性，因为它确实在完整赋值时检测到冲突。
...
然而，通常这两个潜在优势并不起主要作用，因为（a）大多数在工业中解决的问题是自动生成的，因此可读性无关紧要，和（b）现实中的约束问题混合了许多类型的约束，因此解决它们仍然是指数级的。当前的观点是，CSP和SAT在运行时间上并不相互主导，这更多的是一个谨慎工程的问题，而不是实质性的。