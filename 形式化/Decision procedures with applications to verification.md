...
设  $B_i$ 是算法3.3.1中循环的第  i  次迭代中的公式  B 。在所有  $i \geq 1$  的情况下，约束  $B_{i+1}$  都比  $B_i$  更强，因为阻塞子句在迭代之间被添加但不被删除。很容易看出，这意味着在解决  B_i  时学习到的任何冲突子句在解决  B_j  时都可以重用，其中  i < j 。事实上，这是增量可满足性的一个特例，大多数现代SAT求解器都支持这种特性。

因此，在第4行调用增量SAT求解器可以提高算法的效率。
